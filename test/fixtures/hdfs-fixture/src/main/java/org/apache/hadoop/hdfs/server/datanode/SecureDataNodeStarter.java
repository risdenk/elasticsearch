...
public class SecureDataNodeStarter implements Daemon {
    ...
    public static SecureResources getSecureResources(Configuration conf)
        ...
        boolean checkPorts = !conf.getBoolean(DFSConfigKeys.IGNORE_SECURE_PORTS_FOR_TESTING_KEY,
            DFSConfigKeys.IGNORE_SECURE_PORTS_FOR_TESTING_DEFAULT);
        ...
        if (checkPorts && ss.getLocalPort() != streamingAddr.getPort()) {
        ...
        if (checkPorts && !SecurityUtil.isPrivilegedPort(ss.getLocalPort()) && isSecure) {
            throw new RuntimeException(
        ...
        if (policy.isHttpEnabled()) {
            ...
            if (checkPorts && localAddr.getPort() != infoSocAddr.getPort()) {
            ...
            if (checkPorts && localAddr.getPort() > 1023 && isSecure) {
                throw new RuntimeException(
